...
amino_list = ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET', 'PHE', 'PRO',
              'PYL', 'SER', 'SEC', 'THR', 'TRP', 'TYR', 'VAL', 'ASX', 'GLX', 'XAA', 'XLE']

amino_short = {'ALA': 'A', 'ARG': 'R', 'ASN': 'N', 'ASP': 'D', 'CYS': 'C', 'GLN': 'Q', 'GLU': 'E', 'GLY': 'G',
               'HIS': 'H', 'ILE': 'I', 'LEU': 'L', 'LYS': 'K', 'MET': 'M', 'PHE': 'F', 'PRO': 'P', 'PYL': 'O',
               'SER': 'S', 'SEC': 'U', 'THR': 'T', 'TRP': 'W', 'TYR': 'Y', 'VAL': 'V', 'ASX': 'B', 'GLX': 'Z',
               'XAA': 'X', 'XLE': 'J'}
...
def create_amino_acids(acids):
    retval = [
        acid_dict[acid_name] if acid_name in amino_list else acid_dict['MET'] if acid_name == 'FME' else acid_dict[
            'TMP'] for acid_name in acids]
...
for n in range(num_prots):
    if not is_empty_pdb(pdb_ids[n]):
        ...
        try:
            ...
            count += 1
            print(f'##count##:{count}')
            # print(f'proteins = {proteins}, adjacencies = {adjacencies}, pnames = {pnames}, pseqs = {pseqs}')
            ...
            print(f'##except##:{pdb_name}, {uniprot_id}, {user_chain}')
